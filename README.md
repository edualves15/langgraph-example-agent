...
- **Provedor LLM configurável por `LLM_PROVIDER`:** `google` (default) / `openai` / `anthropic`
  / `ollama` / `custom` — o **único ponto** de LLM é `app/services/llm_service.py`. Modelos sem
  function calling (ou sem streaming) nativo são atendidos por uma **camada de emulação** opcional.
...
cp .env.example .env             # defina ao menos LLM_API_KEY (provider default: google)
...
Rotas: `POST /stream` (SSE) · `POST /invoke` (JSON) · `GET /health`.
...
oficial `RunAgentInput` (aparece no **Schemas**, com exemplo no "Try it out"). O `/stream`
devolve um stream **SSE** — não modelável como corpo único —, documentado como **catálogo de
eventos** no `200`; o `/invoke` devolve o `AgentInvokeResponse` (JSON).
...
|---|---|---|
| `POST /stream` | `app/routers/agent.py` | Stream SSE: `agent.clone().run(input)` + `EventEncoder`, com **wrap de `RUN_ERROR`** |
| `POST /invoke` | `app/routers/agent.py` | JSON síncrono: agrega os eventos do run num `AgentInvokeResponse` (`content`/`state`/`interrupt`) |
...
- `app.js` — cria `HttpAgent({ url: "/stream" })`, assina os eventos, mantém uma bolha por
...
app/
  ...
  routers/           agent.py (POST /stream, POST /invoke) · health.py (GET /health)
  agent/             ENGINE genérico: graph.py · state.py (AgentState base) · domain.py (contrato
                       Domain) · tool_emulation.py (camada) · prompts/ (system.md + composição)
  registries/        tool_registry.py (só capabilities genéricas: calendário + math + web)
  services/          llm_service.py (provider por nome) · mcp_service.py (load/merge/get + isolamento)
...
Prefixos: `LLM_*` (provider) · `AG_UI_*` (protocolo/wire) · `APP_*` (aplicação/servidor).

| Variável | Default | Descrição |
|---|---|---|
| `LLM_PROVIDER` | `google` | `google`/`openai`/`anthropic`/`ollama`/`<custom>` (mapeamento em `llm_service.py`) |
| `LLM_API_KEY` | — | Chave do provider (alias aceito: `GEMINI_API_KEY`) |
| `LLM_BASE_URL` | — | Base URL p/ `ollama` e p/ o provider `custom` (vazia nos cloud) |
...
| `APP_CORS_ORIGINS` | `*` | Origens permitidas via CORS (CSV). `*` libera todas |
| `APP_MAX_BODY_BYTES` | `2000000` | Tamanho máx. do corpo (bytes). `0` desabilita |
| `APP_MCP_STARTUP_TIMEOUT` | `15` | Timeout (s) por servidor MCP no startup; o que estoura é pulado. `0` desabilita |
...
`_DEFAULT_MODELS` (`app/services/llm_service.py`). Os pacotes de `openai`/`anthropic`/`ollama`
são **extras** opcionais do `pyproject` (`pip install '.[openai]'`); `google` já está nas deps.

### Provedor LLM e contrato

`app/services/llm_service.py` é o único ponto de LLM: `get_llm()` escolhe a classe pelo nome do
provider e, opcionalmente, embrulha o modelo na **camada de emulação**
(`app/agent/tool_emulation.py`) quando `LLM_TOOL_EMULATION=true`. O que o provider precisa
oferecer:

| Capacidade | Necessária? | Sem ela |
|---|---|---|
| **Tool calling** (nativo ou emulado) | **Sim** | Toda a interação (tools de backend/frontend, HITL) depende de `AIMessage.tool_calls` |
| **Structured output** nativo | Não | Não é usado (estado vem de `Command`; sugestões são texto) |
| **Streaming** de tokens | Não | Backend/estado seguem; o texto do chat é emitido pela camada de emulação (hook oficial) |

A **camada de emulação** é transparente ao grafo (mesma interface `bind_tools`): injeta as
ferramentas no prompt, faz parse da saída em `tool_calls` e emite o texto pelo hook oficial da
lib — habilitando modelos sem function calling e/ou sem streaming nativo (best-effort).
...
- **Resiliência do stream:** o `/stream` aborta limpo no disconnect do cliente
