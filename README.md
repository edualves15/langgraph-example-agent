...
- **Provedor LLM configurável por `LLM_PROVIDER`:** `google` (default) / `openai` / `anthropic`
  / `ollama` / `custom` — o **único ponto** de LLM é `app/services/llm_service.py`.
...
provider. Além dos embutidos (`google`/`openai`/`anthropic`/`ollama`/`custom`), o dev pode
**cadastrar um provider próprio** sem editar o if-ladder:
...
|---|---|---|
| **Tool calling** nativo | **Sim** | Toda a interação (tools de backend/frontend, HITL) depende de `AIMessage.tool_calls`. O LangGraph não emula — quem provê é a integração do provider. Universal nos providers reais. |
| **Structured output** nativo | Não | Não é usado (estado vem de `Command`; sugestões são texto) |
| **Streaming** de tokens | Não (transparente) | O backend/estado seguem por `AIMessage.tool_calls`; o texto do chat usa os eventos de streaming, que todos os providers reais emitem |
